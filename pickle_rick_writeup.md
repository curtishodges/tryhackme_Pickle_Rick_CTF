...
The nMap resaults show that there is two ports running on the system, one is SSH and the other is http.

As i have not got any credidentials to try on the SSH i will focus on the web-page running on the http port.

Upon visting the IP a simple web page is loaded, which has an image and a message to Morty.
...
The command above, initaites gobuster into it's directory/file enumeration mode, the -w option links to the wordlist that gobuster will use, the -u is the url of the webpage and finally -x allows us to target specific file xtentions, in this instance i specified the files php, html and txt.

![gobuster_resault](file:///home/kraken/ctf_writeups/pickle_rick_ctf/2021-10-10_19-49.png)

Gobuster was able to find serveral intresting pages for us to enumerate further. 
...
As the index.html, is the home page nothing of intrest was there. However, navigating to **login.php** gave us a login page. Potentially the one that the username we found earlier is linked to. Could be a possible page to try hyrda brute forcing.
...
Naviagting to **robots.txt**, we are greeeted with a blank page with the word
> Wubbalubbadubdub
...
The hunch was right, the combonation of the username found in the source code and the password found in robots.txt gives us access to the portal.php page! 

> Username: RickRul3s

> Password: Wubbalubbadubdub

For sake of complition, i attempted the above credtianls over SSH however this approach didn't work and as such the focus now goes onto the portal.php page.
...
I started with entering nothing into the command panel, to see if any errors or information came about. Nothing came up, except a blank box. This indicates that commands can be ran, now it is figuring out which commands can be ran. Refering back to information that nMap found earlier, it is a safe assumption that the system is Ubuntu therefore we can base our commands around that system, 
...
Bingo, serveral files instantly stand out. Focusing on the **Sup3rS3cretPickl3Ingred.txt** file first. I attempted to cat the file out, however this returned that the command was disabled, the make it harder for PICKLEEEE RICCCKKK... shame
...
I first identiifed if there was python3 on the system, running the command which python3 returned that there was python3 on the system.
...
Sucess, we managed to gain a reverse shell into the system. 
...
Awesome, just like that we are able to find the second ingredient!

> Command: cat "second ingredients"

(note: As the file contains a space wrapping the file within " " is needed in order for linux to reconise the file)
